...
# --- File state machine (CONTEXT.md > File States, Requeue Eligibility) ---

class FileState:
    """The four states a File passes through."""
...
# The retry-cap boundary, defined once. A File is *terminal-FAILED* once its
# attempts reach the cap (needs explicit human re-queue); below the cap it is
# still auto-retryable. Both renderings below derive from this one rule: the SQL
# WHERE fragment for set-based queries, and the Python predicate for a record
# already fetched. Callers must never re-spell this condition inline.
TERMINAL_FAILED_WHERE = "state = 'FAILED' AND attempt_count >= ?"
RETRYABLE_FAILED_WHERE = "state = 'FAILED' AND attempt_count < ?"


def is_terminal_failed(record: "FileRecord", retry_cap: int) -> bool:
    """Whether a File has exhausted its retries and needs explicit re-queue."""
    return record.state == FileState.FAILED and record.attempt_count >= retry_cap


# One hydration from an audit row to a FileRecord, in two widths. `_FULL_COLUMNS`
# carries every field (used wherever provenance/manifest data matters);
# `_CORE_COLUMNS` carries the identity + state fields the requeue paths need.
_FULL_COLUMNS = (
    "id, filename, source_dir, content_hash, state, attempt_count, error_message,"
    " worker_id, claimed_at, row_count, source_type, source_name, producer,"
    " external_run_id, manifest_payload, manifest_version, manifest_started_at,"
    " manifest_finished_at, manifest_record_count, source_range,"
...
def _hydrate_full(row) -> "FileRecord":
...
def _hydrate_core(row) -> "FileRecord":
    return FileRecord(
        id=row[0], filename=row[1], source_dir=row[2], content_hash=row[3], state=row[4],
        attempt_count=row[5], error_message=row[6], worker_id=row[7], claimed_at=row[8],
    )
...
def find_file_by_hash(db: Database, content_hash: str) -> Optional[FileRecord]:
    cursor = db.execute(
        f"SELECT {_FULL_COLUMNS} FROM etl_file_audit WHERE content_hash = ?",
        [content_hash],
    )
    row = cursor.fetchone()
    if row is None:
        return None
    return _hydrate_full(row)
...
def reset_eligible_failed(db: Database, retry_cap: int) -> int:
    ...
    cursor = db.execute(
        ...
        f" WHERE {RETRYABLE_FAILED_WHERE}",
        [_now(), retry_cap],
    )
...
def find_files_by_filename(db: Database, filename: str) -> list["FileRecord"]:
    ...
    cursor = db.execute(
        f"SELECT {_FULL_COLUMNS} FROM etl_file_audit WHERE filename = ?",
        [filename],
    )
    return [_hydrate_full(row) for row in cursor.fetchall()]
...
def find_terminal_failed_by_filename(
    ...
    cursor = db.execute(
        f"SELECT {_CORE_COLUMNS} FROM etl_file_audit"
        f" WHERE filename=? AND {TERMINAL_FAILED_WHERE}",
        [filename, retry_cap],
    )
    return [_hydrate_core(row) for row in cursor.fetchall()]
...
def list_terminal_failed(db: Database, retry_cap: int) -> list["FileRecord"]:
    ...
    cursor = db.execute(
        f"SELECT {_CORE_COLUMNS} FROM etl_file_audit"
        f" WHERE {TERMINAL_FAILED_WHERE} ORDER BY updated_at DESC",
        [retry_cap],
    )
    return [_hydrate_core(row) for row in cursor.fetchall()]
...
def requeue_all_terminal_failed(db: Database, retry_cap: int) -> int:
    ...
    cursor = db.execute(
        "UPDATE etl_file_audit SET state='PENDING', attempt_count=0, error_message=NULL,"
        f" worker_id=NULL, updated_at=? WHERE {TERMINAL_FAILED_WHERE}",
        [_now(), retry_cap],
    )
    return cursor.rowcount
